...
import { shellMarkup, terminalOptions, managementTabs } from "./presentation.js";
...
const state = {
  ...
  fit: null,
  connection: new ConsoleSession(),
  get session() { return this.connection.id; },
  resizeObserver: null,
  operation: null,
  fingerprint: null,
  busy: false
...
const demoBackend = {
  InitialState: async () => ({ version: "2.2.0-dev", platform: "windows/amd64", state_root: "C:\\Users\\admin\\AppData\\Roaming\\bastionctl", profiles: ["minimal", "web", "docker-host", "wireguard", "database"], servers: [demoServer] }),
...
function shell() {
  ...
  document.querySelector("#management-close").addEventListener("click", closeManagement);
  document.querySelector("#management-dialog").addEventListener("cancel", (event) => {
...
function renderServers() {
  ...
  list.innerHTML = state.servers.map((server) => `
    <button class="server-item ${server.id === state.selected ? "selected" : ""}" data-id="${esc(server.id)}">
      ...
      <span class="server-status">${esc(statusText(server))}</span>
    </button>`).join("");
...
function renderTopbar() {
  ...
  if (!server) {
    bar.innerHTML = '<div><h2>Консоль</h2><p>Сервер не выбран</p></div>';
    return;
  }
  bar.innerHTML = `
    <div class="console-title"><h2>${esc(server.name)}</h2><p>${esc(server.target)}:${server.port} · <span aria-live="polite">${state.connection.starting ? "Подключение…" : state.session ? "Подключено" : "Отключено"}</span></p></div>
    <div class="button-row"><button class="secondary" id="terminal-clear">Очистить</button><button class="secondary" id="manage-server">Управление</button><button class="primary" id="terminal-toggle" ${state.connection.starting ? "disabled" : ""}>${state.session ? "Отключить" : server.bootstrap_pending ? "Первый вход" : "Подключить"}</button></div>`;
  document.querySelector("#terminal-clear").addEventListener("click", () => state.terminal?.clear());
  document.querySelector("#manage-server").addEventListener("click", () => openManagement());
  document.querySelector("#terminal-toggle").addEventListener("click", async () => {
    if (state.session) {
      const session = state.session;
      try {
        await call("StopTerminal", session);
        state.connection.end(session);
        renderTopbar();
      } catch (error) { showError(error); }
    } else openConnect(server, server.bootstrap_pending);
  });
}

function openManagement(tab = state.tab) {
...
function closeManagement() {
  ...
  if (state.session) state.terminal?.focus();
}
...
function renderTabs() {
  ...
  nav.innerHTML = managementTabs.map(([id, label]) => `<button class="tab ${state.tab === id ? "active" : ""}" role="tab" aria-selected="${state.tab === id}" data-tab="${id}">${label}</button>`).join("");
...
function renderContent() {
  ...
  const server = selectedServer();
  if (!server || !state.managementOpen) return;
  const views = { overview: overviewView, security: securityView, users: usersView, services: servicesView, history: historyView, settings: settingsView };
...
function step(number, label, done, action, disabled = false) {
  return `<button class="step ${done ? "done" : ""}" data-step="${action}" ${disabled ? "disabled" : ""}><span>${number}</span><strong>${label}</strong><small>${done ? "Готово" : "Открыть"}</small></button>`;
}
...
function securityView(content) {
  content.innerHTML = `
    ...
    <div class="action-grid">
      <button class="action-card" data-action="audit"><strong>Аудит</strong><small>Только чтение, без изменений</small></button>
      <button class="action-card" data-action="plan"><strong>План</strong><small>Показать предстоящие изменения</small></button>
      <button class="action-card" data-action="apply"><strong>Применить</strong><small>Изменить только управляемую политику</small></button>
    </div>
...
function terminalView() {
  const server = selectedServer();
  document.querySelector("#terminal-host").hidden = !server;
  document.querySelector("#terminal-empty").hidden = Boolean(server);
  if (server) ensureTerminal();
}

function fitTerminal() {
  const host = document.querySelector("#terminal-host");
  if (host?.clientWidth && host?.clientHeight) state.fit?.fit();
}
...
function ensureTerminal() {
  ...
  if (!host) return;
  if (!state.terminal) {
    state.terminal = new Terminal(terminalOptions);
    ...
    // This host is permanent. Opening another section never reopens xterm.
...
async function startConsoleSession(server, method, request, message) {
  let generation;
  try { generation = state.connection.begin(server.id); }
  ...
  try {
    ...
    state.connection.started(id, generation);
  } catch (error) {
    if (generation !== state.connection.generation) return;
    state.connection.failed(generation);
...
async function openConnect(server, bootstrap) {
  if (state.connection.busy) return toast("Сначала отключите активную консоль", "warning");
  ...
  modal({
    ...
    onSubmit: async () => {
      ...
      closeModal();
      await startConsoleSession(server, bootstrap ? "StartBootstrap" : "StartTerminal",
        { password, passphrase }, `Подключение к ${bootstrap ? server.bootstrap_target : server.target}…`);
    }
  });
}
...
async function servicesView(content, server) {
  ...
  try {
    ...
    if (!state.managementOpen || state.tab !== "services" || state.selected !== server.id) return;
...
async function historyView(content, server) {
  ...
  try {
    ...
    if (!state.managementOpen || state.tab !== "history" || state.selected !== server.id) return;
...
async function settingsView(content, server) {
  ...
  try {
    ...
    if (!state.managementOpen || state.tab !== "settings" || state.selected !== server.id) return;
...
async function installServer(server) {
  if (state.connection.busy) return toast("Сначала отключите активную консоль. Установка откроет отдельную сессию.", "warning");
  ...
  modal({
    ...
    onSubmit: async () => {
      ...
      if (!confirmation) return;
      await startConsoleSession(server, "StartInstall", { confirmation, passphrase },
        "Загружаем встроенный Ubuntu-компонент. При запросе [sudo] password введите пароль администратора.");
    }
  });
}
...
function modal({ title, body, submit, onSubmit, onCancel = null, dangerous = false, wide = false }) {
  ...
  closeModal();
  layer.innerHTML = `<dialog class="modal ${wide ? "wide" : ""}" aria-labelledby="modal-title"><header><h2 id="modal-title">${esc(title)}</h2><button class="secondary modal-close">Закрыть</button></header><div class="modal-body">${body}</div><footer><button class="secondary modal-cancel">Отмена</button><button class="${dangerous ? "danger-button" : "primary"} modal-submit">${esc(submit)}</button></footer></dialog>`;
  ...
  const submitButton = layer.querySelector(".modal-submit");
  ...
  dialog.querySelector("form")?.addEventListener("submit", (event) => { event.preventDefault(); submitButton.click(); });
  dialog.addEventListener("cancel", (event) => { event.preventDefault(); cancel(); });
  dialog.addEventListener("click", (event) => {
    const bounds = dialog.getBoundingClientRect();
    if (event.target === dialog && (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom)) cancel();
  });
  dialog.showModal();
...
function closeModal() {
  const layer = document.querySelector("#modal-layer");
  layer.querySelector("dialog")?.close();
  layer.innerHTML = "";
}
...
function setBusy(enabled, text = "Выполняется…") {
  ...
  if (enabled) {
    ...
    overlay.innerHTML = `<strong role="status">${esc(text)}</strong>`;
  } else if (overlay) { overlay.close(); overlay.remove(); }
}
...
function toast(message, type = "info") {
  ...
  const dialogBody = document.querySelector("#modal-layer dialog[open] .modal-body")
    || (state.managementOpen ? document.querySelector("#content") : null);
  if (dialogBody) dialogBody.prepend(item);
  else document.querySelector("#toasts").append(item);
...
async function refresh(renderAfter = true) {
  try {
    ...
    if (!state.selected || !state.servers.some((item) => item.id === state.selected)) {
...
function bindEvents() {
  ...
  events.EventsOn("terminal:event", (event) => {
    if (!state.connection.accept(event)) return;
	if (event.kind === "data") {
    ...
    if (event.kind === "closed") {
      ...
      state.connection.end(event.session_id);
      renderTopbar();
    }
  });
