...
- Восстановлен минималистичный интерфейс: серверы и «Добавить» слева, постоянная
  SSH-консоль справа. Логотип, декоративные иконки и цветной акцент убраны.
- Приложение оформлено в нейтральных серых оттенках. Фон консоли имеет 15%
  прозрачности одним CSS-слоем, без ослабления яркости текста.
- Все административные функции сохранены за кнопкой «Управление»; открытие
  разделов не пересоздаёт xterm. ResizeObserver обновляет размер удалённого PTY.
- Добавлена модель состояния SSH-сессии с защитой от поздних событий/ответов,
  повторного старта и переключения сервера при незавершённом подключении.
- Диалоги используют штатное управление фокусом и Escape; кнопки закрытия
  текстовые, ошибки видны внутри открытого диалога.
- Добавлены единый VERSION, frontend-контрактные тесты и проверяемая публикация
  GitHub Release после CI для Ubuntu и Windows. Сборка выполняется дважды;
  файлы черновика проверяются по размеру и SHA-256 перед публикацией.
- Изменения восстановлены новым коммитом поверх 2.1.0: локальный `a47f17e`
  был недоступен после очистки временной среды. Серверные политики не изменены.

## 2.1.0 — 2026-08-31

- Windows release стал одним `bastionctl.exe`: статические Ubuntu-компоненты
  amd64 и arm64 собираются из того же commit/version и встраиваются через
  `go:embed`; отдельный ZIP и соседние серверные файлы больше не нужны.
- Перед передачей приложение read-only проверяет Ubuntu и архитектуру, выводит
  имя, размер, SHA-256 и пути установки, затем требует точное подтверждение
  `INSTALL <server-id>`.
- Встроенный компонент отправляется напрямую из памяти по pinned-host-key SSH.
  Размер ограничен, число принятых сервером байт и SHA-256 проверяются до sudo;
  временные пути, backup, rollback, `visudo` и итоговый version probe сохранены.
- Убран ручной путь к Ubuntu-бинарнику из Windows UI. Успешной считается только
  установка компонента той же версии, что и открытое приложение.
- Исправлен release pipeline Wails: Windows-файл теперь обязательно собирается
  с production build tag. Добавлена проверка минимального размера PE и Windows
  CI-тест, который открывает оба встроенных payload через application package.
- README, security model, architecture, roadmap, CI и release metadata
  обновлены для single-file дистрибутива.
